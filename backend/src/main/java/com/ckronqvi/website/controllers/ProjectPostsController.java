...
public class ProjectPostsController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteProjectPost(@CurrentUser User user, @PathVariable("id") Long id) {
        projectPostsService.deleteProjectPost(user, id);
        return ResponseEntity.noContent().build();
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> updateProjectPost(@CurrentUser User user, @PathVariable("id") Long id,
            @RequestBody ProjectPostRequestDTO projectPostUpdateDTO) {
        projectPostsService.updateProjectPost(user, id, projectPostUpdateDTO);
        return ResponseEntity.noContent().build();
    }
}
